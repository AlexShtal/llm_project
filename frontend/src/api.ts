type GenerateResponse =
  | string
  | { response?: string; text?: string; message?: string };
...
export async function generateText(prompt: string, token: string): Promise<string> {
  const data = await parseResponse<GenerateResponse>(
  ...
  return (
    data.response ?? data.text ?? data.message ?? JSON.stringify(data, null, 2)
  );
}
